...
if [[ ${#} -ne 2 && ${#} -ne 4 ]]; then
    ...
    echo "!> \file code_version.f90  Source file generated by CMake to report the evTools version used" > ${F90FILE}
    echo "" >> ${F90FILE}
    
    echo "!***********************************************************************************************************************************" >> ${F90FILE}
    echo "!> \brief  Subroutine generated by CMake to report the evTools version used" >> ${F90FILE}
    echo "" >> ${F90FILE}
    echo "subroutine print_code_version(unit)" >> ${F90FILE}
    echo "  implicit none" >> ${F90FILE}
    echo "  integer, intent(in) :: unit" >> ${F90FILE}
    if [ ${RELEASE} == 'yes' ]; then
	echo "  character :: code_version*(99) = 'v"`cat .version`"'" >> ${F90FILE}
	echo "  character :: release_date*(99) = '"`date +"%F"`"'" >> ${F90FILE}
	
	echo "  " >> ${F90FILE}
	echo "  write(unit,'(A)', advance='no') 'evTools '//trim(code_version)//' ('//trim(release_date)//') - evtools.sf.net'" >> ${F90FILE}
    else
	...
	echo "  " >> ${F90FILE}
	echo "  write(unit,'(A)', advance='no') 'evTools '//trim(code_version)//', compiled on '//trim(compile_date)//' with '// &" >> ${F90FILE}
